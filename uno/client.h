...
<html>
  ...
  <body>
    ...
    <script>
      ...
      function drawRadarSweep(distances) {
        const canvas = document.getElementById("radarCanvas");
        if (!canvas || !Array.isArray(distances)) return;
        ...
        const RADAR_LEFT_ANGLE = 125;
        const RADAR_RIGHT_ANGLE = 55;
        const angleStep = 5;

        for (let i = 0; i < distances.length; i++) {
          const angleDeg = RADAR_LEFT_ANGLE - i * angleStep;
          const distance = Math.min(distances[i], 1000);
          const angleRad = (angleDeg * Math.PI) / 180;
          const relAngle = angleRad - Math.PI;
